	...
	public boolean parseCommunityBoardCommand(String command, Player player)
	{
		...
		else if (command.startsWith("_bbsheal"))
		{
			...
			else
			{
				...
				player.setVitalityPoints(150000,false);
				if (player.hasPet())
				{
		...
		else if (command.startsWith("_bbsclean"))
		{
			final String page = command.replace("_bbsclean;", "");
			if (player.getInventory().getInventoryItemCount(Config.COMMUNITYBOARD_CURRENCY, -1) < (Config.COMMUNITYBOARD_HEAL_PRICE))
			{
				player.sendMessage("Not enough currency!");
			}
			else
			{
				...
				player.sendMessage("You used clean" + player.getStat().getCriticalHit());
			}
